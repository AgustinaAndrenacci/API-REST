...
// Verifica si el usuario logueado es el destinatario del mensaje
 const isDestinatario = async (req, res, next) => {
  try {
    const mensaje = await Mensaje.findById(req.params.id);
    if (!mensaje) {
      showErrorMessage(res, 404, "Mensaje no encontrado");
    ...
    if (mensaje.destinatario.toString() !== req.user._id.toString()) {
      showErrorMessage(res, 403, "No autorizado: no eres el destinatario" );
      //return res.status(403).json({ error: "No autorizado: no eres el destinatario" });
    }
    next();
  } catch (error) {
    showErrorMessage(res, 500, err.mensaje||"Error en la verificación de destinatario" );
    //res.status(500).json({ error: "Error en la verificación de destinatario" });
  }
};

 const isRemitenteOrDestinatario = async(req, res, next) => {
  try {
    const userId = req.user.id; // desde tu auth middleware
    const mensajeId = req.params.id;

    if (!mensajeId) {
       showErrorMessage(res, 400, "ID de mensaje requerido." );
    ...
    if (!esRemitente && !esDestinatario) {
       showErrorMessage(res, 403, "No autorizado" );
    ...
    //return res.status(500).json({ error: "Error interno del servidor." });
     showErrorMessage(res, 500, err.messaje||"Error en la verificación de remitente y destinatario");
  }
}

module.exports = {isRemitenteOrDestinatario, isDestinatario,isRemitente };